...
@class middleware
@static
@uses utils, *debug, *express-combo
@extensionfor yui
*/
module.exports = {
    ...
    **/
    exposeSeed: function () {
        ...
        function composeSeedList(yui) {
            ...
            function flush() {
                if (stack.length > 0) {
                    seedCache.push(prevGroup.comboBase + stack.join(prevGroup.comboSep));
                }
                stack = [];
            }
            ...
            for (i = 0; i < modules.length; i += 1) {
                ...
                if (groupConfig) {
                    ...
                    if (groupConfig.combine) {
                        ...
                        seedCache.push(groupConfig.base + path);
                    }
        ...
        function composeStaticHandlers(yui) {
            ...
            for (group in groups) {
                if (groups.hasOwnProperty(group) && groups[group].local && yui._groupFolderMap && yui._groupFolderMap[group]) {
                    handlers.push(combo.folder(group, yui._groupFolderMap[group], utils.extend({}, options)));
            ...
            if (localGroupConfig && localGroupConfig.comboSep && localGroupConfig.comboBase) {
                handlers.push(combo.combine(utils.extend({}, options, {
