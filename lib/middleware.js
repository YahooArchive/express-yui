...
that could be used to boot `YUI` in the client runtime.
    ...
    var app = express();
    app.applyConfig({ fetchCSS: false });
    app.use(yui.expose());
...
module.exports = {
    ...
    **/
    ...
    /**
    Serves YUI Core Modules from the same origin as to where the application
    is hosted.

    Here is an example on how to use it:

        app.yui.serveCoreFromOrigin({}, {});
        ...
        function composeStaticHandlers(yui) {
            ...
            for (group in groups) {
                if (groups.hasOwnProperty(group) && groups[group].local && yui._groupFolderMap && yui._groupFolderMap[group]) {
                    handlers.push(middleware.folder(group, yui._groupFolderMap[group], options));
                }
            }
