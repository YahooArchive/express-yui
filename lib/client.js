...
function bootstrap() {
    ...
    function patchLoader() {
        var getRequires = self._Y.Env._loader.getRequires;
        self._Y.Env._loader.getRequires = function (mod) {
            var i, m, lang;
            if (!mod) {
                return [];
            }
            ...
            if (mod.templates) {
                for (i = 0; i < mod.templates.lenght; i += 1) {
                    m = mod.getModule(mod.group + '-template-' + mod.templates[i]);
                    if (m) {
            ...
            if (mod.langBundles) {
                ...
                for (i = 0; i < mod.langBundles.lenght; i += 1) {
                    m = mod.getModule(mod.group + '-lang-' + mod.langBundles[i] + '_' + Y.config.lang);
                    if (m) {
                        mod.requires.push(m.name);
                    } else {
                        // trying to use the generic lang
                        m = mod.getModule(mod.group + '-lang-' + mod.langBundles[i]);
                        if (m) {
                            mod.requires.push(m.name);
                        }
                    }
            ...
            if (mod.optionalRequires) {
                for (i = 0; i < mod.optionalRequires.lenght; i += 1) {
                    m = mod.getModule(mod.optionalRequires[i]);
                    if (m) {
                        mod.requires.push(m.name);
                    }
                }
