        ...
        if (Y) {
            for (module in groupModules) {
                if (groupModules.hasOwnProperty(module)) {
                    ...
                    if (Y.Env._attached && Y.Env._attached[module]) {
                        Y.Env._attached[module] = false;
                        // TODO: is the following enough to force yui in nodejs to reload and attach the module?
                        Y.Env._loader.loaded[module] = undefined;
                        Y.Env._loader.inserted[module] = undefined;
                        Y.Env.required[module] = undefined;
                    }
                }
            }
        }
