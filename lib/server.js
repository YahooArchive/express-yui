...
module.exports = {
    ...
    **/
    ...
    /**
    Applies a new conditional loading exception based on a trigger module.
    ...
    Any condition added thru this mechanism on the server side will be
    propagated to the client side runtime as well, and there is not need to
    apply it again in the client code. If you use `app.yui.use()` to boot the
    application in the client side, the conditions will be taken in consideration
    automatically.

    @method applyCondition
    @param {String} trigger The module name that should trigger this conditional behavior.
    @param {Object} The condition definition.
    @public
    @chainable
    **/
    applyCondition: function (trigger, cond) {
        var cs = this.config().conditions;
        cs[trigger] = cs[trigger] || [];
        cs[trigger].push(cond);
        if (this._Y) {
            cs = this._Y.config.conditions;
            cs[trigger] = cs[trigger] || [];
            cs[trigger].push(cond);
        }
        return this;
