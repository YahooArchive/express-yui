/*
 * Copyright (c) 2013, Yahoo! Inc.  All rights reserved.
 * Copyrights licensed under the New BSD License.
 * See the accompanying LICENSE file for terms.
 */

/*jslint node:true, nomen: true */

/**
The `yui.groups` extension provides a set of features
to expose YUI groups to the client so that the loader has the
correct configuration to load required modules.

@module yui
@submodule groups
**/

'use strict';

var libfs      = require('fs'),
    libpath    = require('path'),
    libvm      = require('vm'),
    utils      = require('./utils'),
    YUI        = require('yui').YUI,

    // module scoped for reuse
    contextForRunInContext;

contextForRunInContext = libvm.createContext({
    require: require,
    module: require('module'),
    console: {
        // log: function () {}
        log: function (s) { console.log(s); }
    },
    window: {},
    document: {},
    YUI: null
});

/**
The `yui.origin` extension that provides some basic configuration
that will facilitate the configuration of YUI Core modules and other
groups to be served from the express app in a form of origin server.


TODO: Docs

@class groups
@static
@uses utils, static
@extension for yui
*/
module.exports = {


    /**
    Serves YUI Core Modules from the same origin as to
    where the application is hosted.

    @method captureYUIModuleDetails
    @private
    @param {Object} loaderConfig
    @param {Object} options express static handler options
    @return {function} express static middlewares
    **/
    _captureYUIModuleDetails: function (res) {
        var file,
            sandbox,
            yui = {},
            key,
            groupName,
            modules;

        file = libfs.readFileSync(res.path, 'utf8');
        // console.log(file);

        sandbox = {
            modown: { }, // namespace it
            Intl: {
                add: function (langFor, lang) {
                    res.yui.langFor = langFor;
                    res.yui.lang = lang;
                }
            },
            merge: function (to, from) {
                return utils.extend(to, from);
            },
            applyConfig: function (config) {
                // console.log(config);
                var groups;

                groups = config.groups;
                if (groups && Object.keys(groups).length > 1) {
                    if (this.modown.groups) {
                        console.error('Groups metadata should only contain at most 1 group per file. Only the first one will be used.');
                        return;
                    }
                }

                this.modown.groups = groups;
            }
        };
        contextForRunInContext.YUI = {
            ENV: {},
            config: {},
            use: function () {},
            add: function (name, fn, version, meta) {
                // console.error(fn.toString());

                yui.name = name;
                yui.version = version;
                yui.meta = meta || {};
                if (!yui.meta.requires) {
                    yui.meta.requires = [];
                }
                if (sandbox) {
                    try {
                        // fn.apply(runSandbox, [runSandbox, yui.name]);
                        fn(sandbox, yui.name);
                    } catch (e) {
                        console.error('failed to run js file ' + res.path);
                        console.error('error: ' + e.message);
                    }
                }
            }
        };
        try {
            libvm.runInContext(file, contextForRunInContext, res.path);
        } catch (e) {
            yui = null;
            console.error('failed to parse js file ' + res.path);
            console.error('error: ' + e.message);
        }
        if (yui) {
            // merge the module meta
            res.yui = utils.extend(res.yui || {}, yui);
            // name, version, meta.requires
            // console.log(res.yui);

            // merge the groups app config
            for (key in sandbox.modown.groups) {
                if (sandbox.modown.groups.hasOwnProperty(key)) {
                    groupName = key;
                    modules = sandbox.modown.groups[key].modules || {};
                    break; // only pick the 1st group name
                }
            }
            res.yui.groups = {
                name: groupName,
                modules: modules
            };

            return res;
        }
    },

    /**

    var config = yui.groups.getGroupConfig(path);

    config.name = "metas"; // name of the meta module
    config.version = "0.0.1"; // version matching the meta module
    config.meta.requires = [ ]; // list of requires for the meta module
    config.groups.name = "app"; // the first group name in the configuration
    config.groups.modules = { }; // the module configuration for the group

    @method getGroupConfig
    @public
    @param {String} path The path to the module JS source
    @return {Object} 
    **/
    getGroupConfig: function (path) {
        var res;

        // TODO: handle invalid path
        res = {
            path: path,
            yui: { }
        };

        this._captureYUIModuleDetails(res);

        // console.log(require('util').inspect(res.yui));
        // console.log(require('util').inspect(res.yui.groups));
        return res.yui;
    }

};
