    ...
    plugin: function (options) {
        ...
        return {
            ...
            bundleUpdated: function (evt, api) {
                ...
                files = Object.keys(evt.files || {}).filter(function (element, index, array) {
                    // producing an array of fullPath values
                    array[index] = evt.files[element].fullPath;
                    // filtering out files based on filder if neded
                    return !self.describe.filter || self.describe.filter(bundle, evt.files[element].relativePath);
                });
