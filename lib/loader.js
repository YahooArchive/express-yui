...
module.exports = {
    ...
    **/
    ...
    _buildsInBundle: function (bundleName, modified, jsonFiles) {
        ...
        for (m in modified) {
            if (modified.hasOwnProperty(m)) {
                file = modified[m].fullPath;
                ...
                if (mod) {
                    ...
                    for (m in modified) {
                        if (modified.hasOwnProperty(m)) {
                            ...
                            if (modified[m].fullPath === jsonFiles[i]) {
                                skip = false;
                            }
                            // if there is a modified .js file in the range, we should not skip
                            if (path.extname(modified[m].fullPath) === '.js' && modified[m].fullPath.indexOf(dir) === 0) {
                                skip = false;
                            }
                        }
                    }
