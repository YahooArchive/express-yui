...
module.exports = {
    ...
    **/
    ...
    /**
    Build the list of urls that forms the yui seed.
    ...
    **/
    getSeedUrls: function () {
        ...
        return this._buildUrls(modules, 'js', utils.FILTERS_MAP);
    },
    ...
    @method buildJSUrls
    @public
    @param {string} modules* One or more module name
    @return {Array} the `src` url for each script tag that forms the seed
    **/
    buildJSUrls: function () {
        return this._buildUrls(arguments, 'js', utils.FILTERS_MAP);
    },

    /**
    Build the list of urls that forms the style of a page.

        var links = app.yui.buildCSSUrls('cssbase', 'flickr@hermes');
    ...
    **/
    buildCSSUrls: function () {
        return this._buildUrls(arguments, 'css', utils.CSS_FILTERS_MAP);
    },
    ...
    _buildUrls: function (modules, ext, filterMap) {
        ...
        for (i = 0; i < modules.length; i += 1) {
            ...
            filter = filter && filterMap && filterMap.hasOwnProperty(filter) ?
                    filterMap[filter] : utils.DEFAULT_FILTER;
