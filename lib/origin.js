...
module.exports = {
    ...
    **/
    serveGroupFromAppOrigin: function (metaPath, loaderConfig, fullpathResolver, options) {
        ...
        var yuiPrefix = options.hasOwnProperty('prefix') ? options.prefix : "/",
            yuiBase = options.hasOwnProperty('root') ? options.root : "",
            group,
            groupName,
            config = this.config(),
            fullpath,
    ...
    **/
    serveCombinedFromAppOrigin: function (options) {
        ...
        // this._map is set in _addGroupToMap
        if (!this._map) {
            throw new Error('Unable to serve combined combo requests. ' +
                            'Make sure you call serve[Core|Group]FromAppOrigin ' +
